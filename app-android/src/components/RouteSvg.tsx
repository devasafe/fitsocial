...
// Traçado do percurso em SVG (fallback multiplataforma quando não há mapa de tiles).
export function RouteSvg({
  ...
  const span = Math.max(Math.max(...lats) - minLat, Math.max(...lngs) - minLng) || 1e-6;

  const project = (p: GeoPoint) => ({
    x: pad + ((p.lng - minLng) / span) * (W - 2 * pad),
    y: pad + (1 - (p.lat - minLat) / span) * (H - 2 * pad),
  });
