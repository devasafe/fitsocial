...
export function ColarOQuadro({
  ...
  aoLer: (dados: { blocos: Bloco[]; box: string | null; quadro: string }) => void;
}) {
  const { token } = useAuth();
  ...
  async function interpretar() {
    ...
    if (limpo.length < 3) {
      notify("Falta o quadro", "Cole o treino da aula para eu montar os blocos.");
      return;
    }
    ...
    try {
      ...
      if (leitura.blocos.length === 0) {
        ...
        notify(
          ...
          leitura.observacao || "O texto fica guardado do jeito que você colou."
        );
      }

      aoLer({ blocos: leitura.blocos, box: leitura.box ?? null, quadro: limpo });
      setTexto("");
      aoFechar();
    } catch (err) {
  ...
  return (
    <Sheet visivel={visivel} aoFechar={aoFechar}>
      ...
      <Txt variant="body" color={colors.text2} style={{ marginTop: 4, marginBottom: spacing.md }}>
      ...
      {lendo ? (
        <EsperaLonga
          ativo
          passos={["Lendo o quadro…", "Separando os blocos…", "Montando os movimentos…"]}
        />
      ) : (
        <>
          <TextInput
          ...
          <View style={{ marginTop: spacing.md }}>
            <Button title="Montar os blocos" onPress={() => void interpretar()} size="lg" glow />
          </View>
        </>
      )}
    </Sheet>
